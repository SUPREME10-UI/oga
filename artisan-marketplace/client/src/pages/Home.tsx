...
const STATS = [
  { value: "2,500+", label: "Verified Artisans", icon: Users },
...
export default function Home() {
  ...
  const { data: featuredArtisans } = trpc.artisans.list.useQuery({ limit: 6 });
            ...
            <form onSubmit={handleSearch} className="flex gap-2 max-w-lg">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  className="pl-10 bg-white border-0 h-12 text-foreground placeholder:text-muted-foreground"
                />
              </div>
            ...
            {(categories ?? []).map((cat) => {
              ...
              return (
                <Link key={cat.id} href={`/search?categoryId=${cat.id}`}>
                  <Card className="card-hover cursor-pointer border border-border hover:border-primary/30 text-center">
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <p className="text-muted-foreground mb-6 leading-relaxed">
                Our interactive map shows artisans in real-time based on their registered location. Zoom in to
                your neighborhood, click on markers to see profiles, and filter by category, rating, or
                availability.
              </p>
              <ul className="space-y-3 mb-8">
                {[
                  "Real-time artisan locations on map",
                  "Filter by category, rating & availability",
                  "GPS detection for instant nearby results",
            ...
            <div className="relative">
              <div className="rounded-2xl overflow-hidden shadow-2xl border border-border aspect-video bg-[oklch(0.90_0.02_75)] flex items-center justify-center">
                <Link href="/map" className="flex flex-col items-center gap-3 text-muted-foreground hover:text-primary transition-colors">
                  <MapPin className="w-16 h-16" />
                  <span className="font-medium">Click to Open Interactive Map</span>
                </Link>
              </div>
              {/* Floating cards */}
              <div className="absolute -top-4 -right-4 bg-white rounded-xl shadow-lg p-3 border border-border">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs font-semibold">Available Now</p>
                    <p className="text-xs text-muted-foreground">143 artisans</p>
                  </div>
                </div>
              </div>
            ...
            {[
              {
                ...
                desc: "Search by service type, location, or browse the interactive map to find artisans near you.",
                icon: Search,
              },
              {
                step: "02",
                title: "Review & Book",
                desc: "Read reviews, check portfolios, and book directly through the platform with a secure deposit.",
                icon: Star,
              },
              {
                step: "03",
                title: "Get It Done",
                desc: "Communicate via chat, confirm details, and get quality work completed by a verified professional.",
                icon: CheckCircle,
              },
            ].map((item) => (
      ...
      <section className="py-16 craft-gradient">
        <div className="container text-center">
          ...
          <p className="text-white/80 mb-8 max-w-md mx-auto">
            Join thousands of customers who have found trusted artisans through ArtisanHub.
          </p>
